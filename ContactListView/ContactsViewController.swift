...
class ContactsViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.searchBar.placeholder = "Please type here..."
    }
...
//        let companyName = contact.organizationName
        if contact.emailAddresses.count > 0{
            let emailAddr = contact.emailAddresses[0]
            
            let email:CNLabeledValue = emailAddr
            //        emails .addObject(email.value as! String)
            let emailStr = String(email.value)
            cell.textLabel?.text = emailStr
        }else{
            cell.textLabel?.text = "No email"
        }
    ...
    private func fetchContacts(name: String) -> [CNContact] {
        ...
        do {
            let request = CNContactFetchRequest(keysToFetch: [CNContactFormatter.descriptorForRequiredKeysForStyle(.FullName), CNContactEmailAddressesKey, CNContactOrganizationNameKey])
